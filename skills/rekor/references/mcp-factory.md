...
## The two building blocks: Tools and Toolsets
...
- [Tool names (`tool_name` / `description_override`)](#tool-names)
...
rekor toolsets upsert invoicing-agent --base my-ws --config '{
  ...
  "tools": [
    ...
    { "tool": "create_payment" },
    { "tool": "get_payment" },
    { "tool": "list_payments", "description_override": "List the payments recorded against an invoice" }
  ],
  "relationships": [
    {
...
Each Tool's generated MCP tool name defaults to its **`id`** — so a Tool with id `search_invoices` surfaces as `search_invoices`, and `get_invoice` as `get_invoice`. Name a Tool for the job it does when you author it (`rekor tools upsert book_slot …`), rather than accepting a generic `update_slots`.

A toolset reference can override two things per surface without touching the Tool:

- **`tool_name`** — rename this Tool *in this toolset* (e.g. `{ "tool": "search_invoices", "tool_name": "find_invoices" }`). Tool names must be unique across the toolset.
- **`description_override`** — replace the Tool's default description for this toolset.

Tool names must be unique across the toolset, so if you reference the same Tool twice (or two Tools that would collide) give one a distinct `tool_name`. **Relationship** tools are named by the inline `name`/`names` map on their entry — `name` sets the base noun (link/list/unlink over it), `names` overrides a specific operation (`{ "create": "assign_payment" }`); every key inside `names` must be a valid relationship operation (`create`/`list`/`delete`), or the override is rejected at config-write.
...
`book_slot` succeeds only on a slot that is still `free`; `manage_slot` updates the same slot unconditionally (e.g. an operator correction). Because each Tool has its own id, both surface as distinct tools with no name collision.
