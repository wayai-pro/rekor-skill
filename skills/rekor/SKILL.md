...
description: |
  ...
  record_type, working in preview and promoting to production, querying documents via SQL,
  ...
  reliably, or interpreting Rekor concepts (base, record_type, document,
...
A record_type is a JSON Schema that defines a document type. Define it in the preview base:
...
rekor sql "SELECT * FROM documents WHERE org_id = {org_id:String} AND base_id = {base_id:String} AND record_type = '<slug>' AND deleted = false" --base <id>--<preview-slug>
```
...
- **RecordType**: A schema (JSON Schema) that defines a document type. No migrations — create at runtime.
- **Document**: A JSON document conforming to a record_type's schema.
...
```bash
rekor sql "SELECT data.invoice_number.:String as num, data.status.:String as status, arraySum(CAST(data.line_items[].amount, 'Array(Float64)')) as total FROM documents WHERE org_id = {org_id:String} AND base_id = {base_id:String} AND record_type = 'invoices' AND deleted = false ORDER BY total DESC" --base my-ws
```
...
- **Deletions are opt-in.** Because deleting a record_type also removes its documents, `push` is additive by default: entities missing from your files are reported but kept. Add `--prune` to delete them.
- **Renaming a record_type is re-seed, not in-place.** A record_type's `id` is its identity and must equal its filename (`record-types/<id>.yaml`), so renaming means updating **both** the filename and the inner `id:` field together (changing only one errors; or delete the inner `id:` so it defaults to the filename). Either way it's a **create-new + orphan-old**, not an in-place rename: the diff shows `+ <new>` / `- <old>`, `push` creates a new **empty** record_type, and the old one (with all its documents) is left untouched — documents do **not** migrate. To rename and keep the data: (1) `push` to create the new empty record_type, (2) re-seed its documents (e.g. `rekor documents upsert`, or batch), then (3) `push --prune` to delete the orphaned old record_type, which cascades its documents. The same filename-is-`id` rule applies to relationship types and the other config entities.
...
Deleting a record_type also removes all of its documents (and any relationships pointing at them); a record_type recreated with the same id starts empty.

### Documents

```bash
rekor documents upsert <record_type> --base <ws> --data <json|@file> [--id <uuid>] [--external-id <id>] [--external-source <src>]
rekor documents get <record_type> <id> --base <ws>
rekor documents query <record_type> --base <ws> [--filter <json|@file>] [--sort <json>] [--fields <list>] [--limit <n>] [--offset <n>] [--external-source <src>]
rekor documents delete <record_type> <id> --base <ws>
...
**Filtering & search.** `query` (REST `GET /documents/<record_type>`, MCP `query_documents`) takes a Filter DSL expression — a condition `{field, op, value}` or an `and`/`or` group of them. Operators: `eq`, `neq`, `gt`, `gte`, `lt`, `lte`, `in`, `not_in`, `like`, `ilike`, `is_null`, `is_not_null`, `has`, and **`search`**. This DSL is the only accepted form — Mongo-style shorthand (`{"data.field":"value"}`, `{"field":{"$in":[...]}}`) is **not** supported.
...
**Tables**: `documents`, `relationships`, `record_types`, `relationship_types`, `bases`, `operations_log`, `organization`
...
- **Hybrid entities (mirrored fields + a rekor-owned overlay) live in ONE record_type.** A field the upstream lacks (a workflow `status`, a tag, a score) belongs on the same document as the mirrored fields — integrated query/filter demands it (you can't `list where status=X` if `status` lives in another record_type). Splitting an entity to separate ownership trades a real query capability for bookkeeping; don't. Exception: an overlay that is itself a richer entity with its own lifecycle/history → a related record_type + relationship. A scalar attribute is never a reason to split.
...
**Keep your own fields on re-sync — `merge`.** By default each re-sync replaces the whole document, so any field you added locally (a workflow status, tag, score, assignment, internal note) is overwritten. Add `"merge":true` to `--hydration` and a re-sync refreshes **only** the fields the source's mapping owns and **preserves** everything else you put on the document — so one record_type can hold both the upstream mirror *and* your local enrichment, and you can still list/filter the entity by your own field (e.g. `status=qualified`). The first sync still creates the record from the mapped fields; a field the upstream later clears is reflected; the merged document is still schema-validated. Example: `--hydration '{"id_path":"record_id","merge":true}'`.
...
rekor triggers create --base <ws> --name <name> --events document.created --record_type-scope appointments \
...
rekor triggers create --base <ws> --name <name> --events document.created,document.updated --record_type-scope appointments \
...
rekor batch --base <ws> --operations '[
  {"type":"upsert_document","record_type":"invoices","data":{"customer":"A","amount":100}},
  {"type":"upsert_document","record_type":"invoices","data":{"customer":"B","amount":200}},
...
Operation types: `upsert_document`, `delete_document`, `upsert_relationship`, `delete_relationship`, `upsert_record_type`, `delete_record_type`
...
```bash
# 1. Author first-class Tools — one record_type + one operation each; the id is the tool name
rekor tools upsert get_invoice    --base my-ws --record_type invoices --operation get
rekor tools upsert list_invoices  --base my-ws --record_type invoices --operation list
...
**Tool reference format**: `--tool <tool_id>` (or `<tool_id>=<surface_name>` to rename it in this toolset). Author the Tool first with `rekor tools upsert <id> --record_type X --operation Y` — one record_type + one operation, its `id` is the agent-facing tool name.
**Relationship spec format**: `rel_type:op1,op2` (operations: `create`, `list`, `delete`)
**Batch spec format**: `record_type_or_rel:op1,op2`

**Advanced control** lives on the first-class **Tools** a toolset references. Author a Tool with the shaping config — `rekor tools upsert <id> --record_type X --operation Y --config '{…}'` (one record_type + one op) — then reference it from the toolset (`--tool <id>`, or a `tools: [{ tool, tool_name?, description_override? }]` entry in the toolset `--config`). The per-Tool knobs, one line each (full grammar + JSON examples in **`references/mcp-factory.md`**):
...
- **`writable_fields`** — the write-side mirror on a `create`/`update` Tool: an allowlist of exactly the fields it may set (least-privilege intent tools) that also generates a rich typed `data` schema from the record_type schema.
...
- **`binding`** — pick which **named write binding** of a proxy source's op a write Tool dispatches to (see External Sources), keeping one canonical record_type whose writes fan out to several endpoints.
...
rekor tokens create --name "client-a-reader" \
  --grants '[{"scope":{"bases":["client-a"],"environments":["production"]},"permissions":["read:documents","read:record_types"]}]'
...
**Permissions**: `read:documents`, `write:documents`, `read:record_types`, `write:record_types`, `read:relationships`, `write:relationships`, `read:attachments`, `write:attachments`, `read:inbound_webhooks`, `write:inbound_webhooks`, `read:triggers`, `write:triggers`, `read:toolsets`, `write:toolsets`, `read:bases`, `write:bases`, `read:audit` (read-only; grants change-history access, admin-gated, not implied by other grants), or `*` for all.
...
rekor report create --title "Upsert 500s on large docs" --description "..." \
  ...
  [--base-id <id>] [--record_type <name>] [--document-id <id>]   # entity pointers aid investigation
...
- **Schema first** — define the record_type schema before writing documents. Validation catches bad data early.
